...
import { Camera, ChevronDown, LayoutDashboard, Pencil, Bell, Heart } from "lucide-react";
import { CLIENT_NAV, SpaceNav } from "./SpaceNav";
import { useAuth } from "../../lib/auth";
import { api } from "../../lib/api";

function mediaKey(userId, kind) {
  return `saacare_client_${kind}_${userId || "guest"}`;
}
...
/**
 * Header profil client (couverture + identité + onglets).
 * Reste visible sur toutes les pages /client/*.
 */
export default function ClientProfileShell() {
  ...
  const ordersStat = stats.find((s) => /commande/i.test(s.label));
  const messagesStat = stats.find((s) => /message/i.test(s.label));
  const notifsStat = stats.find((s) => /notif/i.test(s.label));
  ...
  const goEdit = () => {
    if (location.pathname.startsWith("/client/profil")) {
      document.getElementById("client-profile-edit")?.scrollIntoView({ behavior: "smooth", block: "start" });
    } else {
      navigate("/client/profil#client-profile-edit");
    }
  };
    ...
    <section className="overflow-hidden rounded-2xl border border-ink-900/8 bg-white shadow-soft">
      <div className="relative">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:gap-5">
              ...
              <div className="min-w-0 pb-1">
                <h1 className="font-display text-2xl font-bold tracking-tight text-ink-900 sm:text-3xl">
                  {user?.fullName || "Client SaaCare"}
                </h1>
                <p className="mt-1 text-sm text-ink-900/55">
                  {[
                    ordersStat ? `${ordersStat.value} commande${Number(ordersStat.value) > 1 ? "s" : ""}` : null,
                    messagesStat ? `${messagesStat.value} message${Number(messagesStat.value) > 1 ? "s" : ""}` : null,
                    notifsStat && Number(notifsStat.value) > 0 ? `${notifsStat.value} notif.` : null,
                    user?.commune || null,
                  ]
                    .filter(Boolean)
                    .join(" · ") || "Espace client SaaCare"}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 lg:pb-1">
              ...
              <Link
                ...
                className={`inline-flex size-10 items-center justify-center rounded-lg transition-colors ${
                  location.pathname.startsWith("/client/favoris")
              ...
              >
                <Heart
                  className="size-4"
                  fill={location.pathname.startsWith("/client/favoris") ? "currentColor" : "none"}
                  aria-hidden="true"
                />
              </Link>
              <Link
                ...
                className={`relative inline-flex size-10 items-center justify-center rounded-lg transition-colors ${
                  location.pathname.startsWith("/client/notifications")
                    ? "bg-teal-50 text-teal-700"
                    : "bg-paper-200 text-ink-900 hover:bg-teal-50 hover:text-teal-800"
                }`}
              ...
              >
                ...
                {Number(notifsStat?.value) > 0 && (
                  <span className="absolute -right-1 -top-1 grid min-w-4 place-items-center rounded-full bg-coral-500 px-1 py-0.5 font-mono text-[0.6rem] font-bold leading-none text-white">
                    {Number(notifsStat.value) > 9 ? "9+" : notifsStat.value}
                  </span>
                )}
              </Link>
              <div className="relative">
        ...
        <div className="hidden border-t border-ink-900/8 lg:block">
          <SpaceNav links={CLIENT_NAV} variant="tabs" />
        </div>
      </div>
    </section>
  );
