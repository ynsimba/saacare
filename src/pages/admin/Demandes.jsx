...
export default function AdminDemandes() {
  ...
  const patchStatus = async (id, status) => {
    ...
    setError("");
    try {
      const data = await api.patchAdminServiceRequest(id, { status });
  ...
  const shown = items.filter(
    (r) =>
      ...
      (!query ||
        [r.reference, r.service, r.commune, r.firstName, r.email, r.phone, r.need]
          .filter(Boolean)
  ...
  return (
    <>
      <Seo title="Demandes" path="/admin/demandes" noindex />
      <DeskHeading as="h1" count={items.length}>
        Demandes web
      </DeskHeading>
      <p className="mt-2 text-sm text-desk-ink/60">Formulaires publics (RequestForm, Diaspora) reçus via l’API.</p>
      ...
      {shown.length ? (
        <div className="mt-5 overflow-x-auto rounded-3xl bg-white p-3 sm:p-4">
          <table className="w-full min-w-[56rem] border-separate border-spacing-y-2 text-left text-sm">
            <caption className="sr-only">Liste des demandes de service</caption>
            <thead>
              <tr className="text-xs font-medium uppercase tracking-wide text-desk-ink/55">
                ...
                <th scope="col" className="px-3 pb-1 font-medium">Service</th>
                ...
                return (
                  <tr key={r.id} className="group">
                    ...
                    <td className={cellBase}>
                      ...
                      <p className="text-desk-ink/55">{r.commune}{r.frequency ? ` · ${r.frequency}` : ""}</p>
                    </td>
                    <td className={cellBase}>
                    ...
                    <td className={`${cellBase} max-w-xs`}>
                      <p className="line-clamp-2 text-desk-ink/70">{r.need}</p>
                    </td>
        ...
        !error && (
          <div className="mt-5">
            <DeskEmpty>Aucune demande pour le moment.</DeskEmpty>
          </div>
        )
      )}
    </>
